...
class ColumnConfiguration(BaseModel):
    ...
    def validators(self):
        return [ALL_VALIDATORS[validator_name] for validator_name in self.validator_strings]
    ...
    def check_validator_is_defined(cls, v):
        for validator in v:
            if validator not in ALL_VALIDATORS:
                ...
                raise ValueError(f"Wrong validator '{validator}' provided. Validator must one of: {keys}")
        return v
