...
def test_validation_of_float_rejects_empty_values_if_allow_empty_set_to_false():
    ...
    assert validation_error == {'is_float_or_not_supplied': "'' was not a valid float or the string 'not-supplied'"}
...
def test_allowed_string_rejects_strings_with_comma():
    ...
    assert validation_error == {'is_allowed_string': "String 'hello,' contains commas. Those are not allowed."}
...
def test_allowed_string_rejects_non_ascii_string():
    ...
    assert validation_error == {'is_allowed_string': "String 'æøå' contains non-ASCII characters. Those are not allowed."} != {'is_allowed_string': ''}
